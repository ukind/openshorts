---
date: 2026-09-06T06:47:45+0700
author: Yogiswara Utama
commit: de92009
branch: main
repository: openshorts
topic: "Marsic1/openshorts fork parity merge — merge mechanics, app.py LLM seam, stage ownership, dashboard seam, infra, remotion, docs"
tags: [design, merge, llm-client, ai-provider, gate-predicate, stage-ownership, mcp, dashboard, infra, remotion, tests]
status: ready
parent: .rpiv/artifacts/research/2026-09-06_06-09-59_marsic-fork-parity-merge.md
last_updated: 2026-09-06T06:47:45+0700
last_updated_by: Yogiswara Utama
---

# Design: Marsic1 fork parity merge

## Summary

Merge Marsic1/openshorts (`marsic/main` @ `b1f5350`, 41 commits) into this repo (`main` @ `de92009`, 15 commits) as **one merge commit** on branch `merge/marsic-parity`; `main` fast-forwards only after verification. Every conflicted file resolves **take-theirs + re-apply our deltas** (developer-confirmed for all 7, CLAUDE.md included). The two independently built LLM systems coexist by **stage ownership**: `ai_provider.py` owns the video pipeline, `llm_client.py` owns the satellite text stages (thumbnail/SaaS/layout) plus MCP BYOK forwarding. One async predicate `ai_backend_available()` replaces both forks' separate job-gate fixes, and a no-double-route contract test pins that no stage can fire two LLM calls.

## Requirements

From the FRD (`.rpiv/artifacts/discover/2026-09-06_05-40-55_marsic-fork-parity-merge.md`):

- Merge `marsic/main` (b1f5350) into `merge/marsic-parity` as a merge commit; `main` fast-forwards only after verification passes.
- Resolve the 7 textual conflicts by taking Marsic1's side as base per file and re-applying our smaller changes.
- `remotion/package.json` resolves to the **union** manifest (theirs + our `zod ^4.3.6`); both lockfiles regenerated and **committed** (their `render-service/Dockerfile` runs `npm ci`).
- Delete `llm_backend.py` and port its gate/config tests into the surviving surface.
- Single gate predicate `ai_backend_available()` (Gemini key OR configured `llm_client` OR per-job provider) used by the job-launch path and `/api/config`; Ollama Cloud and any OpenAI-compatible base URL count as endpoint.
- Stage ownership enforced: `ai_provider.py` = video pipeline; `llm_client.py` = satellites + MCP BYOK; our duplicate clip score/detail routes in `main.py` deleted as superseded.
- `docker-compose.yml` defaults CPU (`GPU: 0` or absent, no `gpus: all`); GPU opt-in via build-arg.
- CLAUDE.md + .env.example document the stage-ownership boundary and the env namespace split.
- All 41 commits' features land config-gated (merged ≠ enabled in prod).
- No permanent `marsic` git remote; the already-fetched `refs/remotes/marsic/main` suffices.

## Current State Analysis

Merge-base `ad8ab59`. Ours: 15 commits / 30 files. Theirs: 41 commits / 121 files (all three counts re-verified in git by the precedent pass). `git merge-tree` shows **7 textual conflicts**: `.env.example`, `CLAUDE.md`, `app.py`, `dashboard/src/App.jsx`, `main.py`, `remotion/package.json`, `remotion/package-lock.json` (add/add). The reference worktree `../openshorts-marsic` (their tree, read-only) backs every anchor below.

### Key Discoveries

- **Gate-line history**: the job-start gate was fixed three times (ours `56707b7` None-env guard; theirs `25222f5` +4/−1 `llm_backend.active()` exemption; the original `0f326fd` upstream merge dropped the exemption once). The predicate replaces the line outright — no fourth hand-fix.
- **Silent rename**: base `dashboard/src/lib/analytics.js` → we renamed to `panel.js`; they modified in place. Only `App.jsx:33` imports `./lib/analytics` in a conflicted file; the other four importers (`AccountPage:5`, `PricingSection:5`, `TopUpModal:4`, `AuthContext:10`) auto-merge keeping our `panel` import + their edits. A grep gate verifies zero `lib/analytics` post-merge.
- **Their satellite heads are broken upstream**: their `/api/thumbnail/titles` (`app.py:5815-5816`) assigns the now-2-tuple `resolve_gemini` return to a scalar and references `provider` before assignment — latent `NameError`. Our re-applied gate pattern fixes it while threading `resolve_llm`.
- **Third direct `gemini_key` reader** (missed by research): `dashboard/src/components/ResultCard.jsx:284` — `geminiApiKey || localStorage.getItem('gemini_key')`. Dead-but-trapping once the plaintext write dies.
- **Plaintext key lifecycle**: their `App.jsx:642` writes `localStorage['gemini_key']` on every mount; our one-way migration (`App.jsx:225-242`) deletes it. The write must die or the migration undoes itself forever.
- **Their env writer is None-safe by construction** (`app.py:2439-2448`: `if gemini_key:` in the openai branch, `gemini_key or ""` in the gemini branch) — our `56707b7` `if api_key:` guard is **not** re-applied.
- **Hunk provenance**: our `resolve_llm`/`LLM_ENDPOINT_HINT`/gate/env-forward come from `ec60f4f`; `_env_llm_config`/`/api/config` keys/`/api/llm/test` come from `f83d555`. Blame confirms **zero drift** since introduction — the re-apply list at HEAD is current.
- **`llm_backend` token audit**: 26 hits in their tree, all on the port/delete list plus the `app.py:2322` comment (same edit as the gate it annotates). Nothing else references the module.
- **MCP allowlist**: research's list missed `x-gemini-model` (their `App.jsx:982` sends it; their `resolve_gemini:139` reads it). The merged allowlist adds **16** headers, not 11.
- **Remotion**: their manifest adds `@remotion/animated-emoji 4.0.447` and **drops `zod`** — it compiles today only because zod hoists transitively from `@remotion/media`. Union manifest + regenerated, committed lockfiles (`npm ci --legacy-peer-deps` flag parity with their Dockerfile).
- **Compose**: their GPU additions (`GPU: "1"` build-arg, `gpus: all` ×2, `hf-cache` volume, NVIDIA renderer env) flip mechanically for the CPU Coolify host; `env_file: [.env]` → `required: false` (developer decision).
- **CI gap**: their `test_game_profiles.py` marks 6 tests `@pytest.mark.asyncio` (:20/:58/:81/:105/:140/:164 — research said 7; recount in the Slice 3 session); CI installs no pytest-asyncio (`.github/workflows/ci.yml:24-26`). One-line CI fix keeps their tests byte-identical.
- **`/api/config` dual-key reconciliation**: both dashboards consume by name — ours `llmConfigured/llmModel/llmBaseUrl` (`AuthContext:152-154` → `App.jsx:779`), theirs `localLlm` (`AuthContext:138` → `App.jsx:841`). Both keys survive, both fed from `_env_llm_config()`, `localLlm` keeps their exact `describe()` dict shape `{"provider","model","baseUrl"}`.

## Scope

### Building

8 vertical slices inside one merge working tree (single merge commit lands in Slice 8):

1. Merge mechanics + `app.py` LLM seam (foundation)
2. `main.py` stage-ownership flip
3. No-double-route boundary contract test + CI pytest-asyncio
4. Dashboard core seam (App.jsx state/persistence/gate/headers + reader patches)
5. Dashboard nav/mounts + AuthContext pin + dashboard package deps
6. Infra: compose CPU flip + `.env.example` union + requirements verify
7. Remotion union manifest + regenerated committed lockfiles
8. Docs + finalize: CLAUDE.md (take theirs + re-apply our corpus), README rewrites, merge commit
...
### D1 (inherited): Take-theirs-then-reapply for all 7 conflicted files
FRD rule, standing for every conflict. Developer explicitly confirmed CLAUDE.md follows the blanket rule too (Step-4 directional question this session): their CLAUDE.md is base, our ~15-commit doc corpus is re-applied, their `llm_backend` section (their `CLAUDE.md:140-151`) is rewritten into the stage-ownership boundary.

### D2: Unified gate predicate `ai_backend_available()`
Lives in `app.py` beside the resolvers (after the re-applied `resolve_llm`) — **not** in `llm_client.py` (the provider leg needs request/header context; `llm_client` is deliberately env-only and import-light). Async, takes `request`, keyword options `gemini_key` and `provider`. Four flat-OR legs: (1) `bool(gemini_key)`; (2) `provider == "openai"`; (3) `await resolve_llm(request) is not None`; (4) `_env_llm_config() is not None`. Job site: their gate condition `provider == "gemini" and not gemini_key` is kept, the third clause becomes the predicate call; self-host raise = `LLM_ENDPOINT_HINT`, billing raise = `gemini_missing_error()` (our old split, `app.py:2190-2193`).

### D3: `/api/config` dual keys, one source
`get_config` gains the predicate's env leg via `_env_llm_config()`: keeps our `llmConfigured/llmModel/llmBaseUrl` and adds their `localLlm` in their exact `describe()` shape `{"provider":"openai","model":...,"baseUrl":...}`, `None` when unconfigured or under billing (the pin inside `_env_llm_config()` covers both).

### D4: Satellite heads adapted to their 2-tuple `resolve_gemini`
Merged pattern per satellite endpoint: `gemini_key, _gemini_model = await resolve_gemini(request)` → `llm_cfg = await resolve_llm(request, task="thumbnail"|"saas")` → gate → primary `thumbnail.`/`saasshorts.` call gains `llm_config=llm_cfg`. Fixes their latent NameError at `app.py:5815-5816`.

Slice-1 extension (E11): the same tuple discipline applies to `/api/edit` (:3331-3335) and `/api/effects/generate` (:4492-4495) — their mid-migration leftovers; without the unpack their AI-effects feature ships dead (always-truthy tuple skips the key gate).

### D5: Stage ownership map
`llm_client.py` owns the `LLM_*` namespace + thumbnail.py + layout_picker.py + mcp_server.py + saasshorts.py reroute (both hunks keep — textually disjoint). `ai_provider.py` owns the video pipeline + deep analysis + semantic scoring + voiceover captions. Deleted: `llm_backend.py`, our `main.py` llm seam (hunks from `ec60f4f`), their dead `score_batch_size`.

### D6: `llm_backend.py` deleted; its surviving semantics re-targeted
Gate exemption → predicate; `/api/config` `localLlm` → `_env_llm_config()`; silent-video gate → `llm_client.active_config() is not None`; tests → ported into `tests/test_llm_endpoints.py` + the slice-3 boundary test. `tests/test_llm_backend.py` deleted with the module.

### D7: MCP allowlist extension
`mcp_server.py:53-55` `_FORWARD_HEADERS` gains 16 of their headers: `x-ai-provider`, `x-gemini-model`, `x-openai-key`, `x-openai-model`, `x-openai-base-url`, `x-game-profile-id`, `x-deep-provider`, `x-elevenlabs-key`, `x-target-clips`, and the seven `x-enable-*` — or their features lose BYOK through MCP.

### D8: Dashboard key lifecycle ours; their direct readers patched
`apiKey` keeps our encrypted init + one-way migration + `geminiKey_v1` persistence; their `:217` init and `:642` plaintext write deleted. `CreateEditProfileModal` receives the key/provider via props (the `VoiceOverPage` mount pattern, their `App.jsx:1990-2003`); `ResultCard:284` drops the dead `localStorage` fallback. `targetClips` → their persistent state wins; our `data.targetClips` advanced path deleted. Gate: `needsAiBackend = !apiKey && !(llmConfigComplete(providerCfg) || llmConfigured || localLlm)`; `geminiOk` deleted.

### D9: History nav union rule
Billing mode keeps `billingEnabled && isSignedIn`; self-host unconditional. Plus `voiceover`, `voice-style-presets`, `game-profiles` tabs, their mount blocks, and the tutorial lock wrapping `goToTab`.

### D10: Compose CPU default + soft env_file
`GPU: "1"` → drop the `args:` block (Dockerfile default `ARG GPU=0`); delete both `gpus: all`; keep `hf-cache`; renderer `NVIDIA_*` env inert-or-removed; `env_file: [.env]` → `env_file: [{ path: .env, required: false }]` (compose ≥2.24); `TZ` kept.

### D11: Remotion union manifest + committed lockfiles
Theirs + `"zod": "^4.3.6"` re-added; both `remotion/package-lock.json` and `render-service/package-lock.json` regenerated with `npm install --legacy-peer-deps` and **committed** (`npm ci` in their render-service Dockerfile hard-fails otherwise).

### D12: Env namespace disposition
Ours kept: `LLM_BASE_URL/LLM_API_KEY/LLM_MODEL/LLM_MODEL_THUMBNAIL/LLM_MODEL_SAAS` (tightened semantics: empty key deactivates). Deleted with no reader: `LLM_PROVIDER`, `LLM_TIMEOUT` (replaced by their `AI_TIMEOUT`, 600→30 s default), `OLLAMA_CONTEXT_LENGTH` (doc-only, dropped). Theirs adopted: `AI_PROVIDER`, `GEMINI_MODEL`, `OPENAI_*`, `AI_TEMPERATURE`, `AI_MAX_TOKENS`, `AI_TIMEOUT`, `DEEP_*`, `ENABLE_*`, `TTS_*`/`VOICEOVER_*`, proxy ops, `HOOK_GROUNDING*`, `TARGET_CLIPS`. Never documented: `GAME_PROFILE_ID`, `USER_ID`, `AI_API_KEY` (near-dead eager alias).

### D13: CI pytest-asyncio added; their tests untouched otherwise
`.github/workflows/ci.yml` pip line gains `pytest-asyncio` (the file is byte-identical in both forks, so the merge auto-merges it clean); their 6 async-marked tests run unmodified — byte-identical tests keep future parity pulls cheap.

### D14: Accepted strictness change, documented
A keyless server on bare `LLM_BASE_URL` reports `llmConfigured: false` post-merge (`config_from` requires base+key+model). Migration: one line (`LLM_API_KEY=<any placeholder>`) or the `AI_PROVIDER=openai` + `OPENAI_BASE_URL` channel (keyless-native). Documented in CLAUDE.md + `.env.example` + merge note.

### D15: Error semantics of flipped stages: theirs wholesale
Exception taxonomy collapses to `AIProviderError` with string-sniff retry classification; `GeminiBlockedError`/bisect ladder lost on pipeline stages; provider jobs report `cost_analysis: None`; `AI_TIMEOUT` 30 s default; temperature 0.7 default. Unification is an explicit non-goal.

### D16: Merge mechanics
`git checkout -b merge/marsic-parity && git merge --no-commit marsic/main` once (Slice 1); the working tree stays uncommitted across slices; one merge commit at Slice 8 with the house message format `Merge <source>: <summary>` plus the `0f326fd`-style per-file resolution policy list; `main` fast-forwards after verification (validate stage, outside this design).

## Architecture

Code fences fill per-slice on approval (Step 6.4). MODIFY entries show only added/changed code; the merged base (their side) is on disk after the merge.

### app.py — MODIFY (their base + 11 re-applied hunks + predicate + satellite heads)
...
### mcp_server.py:51-55 — MODIFY (allowlist extension)
...
### tests/test_llm_endpoints.py — MODIFY (clean-slate extension + gate/config port)

(a) `_clean_slate` delenv tuple gains: `"AI_PROVIDER", "OPENAI_API_KEY", "OPENAI_MODEL", "OPENAI_BASE_URL", "GEMINI_API_KEY", "GEMINI_MODEL", "LLM_PROVIDER"`.
...
### main.py — MODIFY (import rewrite, our seam deleted, silent-video gate retarget)

Base = their `main.py` wholesale (`git checkout --theirs main.py`). None of our `ec60f4f` seam hunks (research hunks 1-9) are re-applied; hunk 10's longer silent-video else-text is the one re-application (inside M4).
...
**M3.** Delete `score_batch_size()` (their :1928-1937) and the single blank line after it (:1938), keeping the two blank lines before it (:1926-1927) as the separator to `get_viral_clips` (:1939). Dead: `SCORE_BATCH = 8` is hardcoded at :2469; the only caller was their `tests/test_llm_backend.py:145-151`, deleted in Slice 1. The `LLM_SCORE_BATCH` env knob dies with it (D12; their CLAUDE.md promise is rewritten in Slice 8).
...
### tests/test_llm_client.py — MODIFY (pipeline-branch block deleted; `_llm_cfg` kept)

Slice-2 scope addition: the research break-list mandates porting the seam tests out of this file, but no slice carried it — without this edit the suite is red between Slices 2 and 3.

Delete the `# --- pipeline branch (slice 2) ---` block (:507-705): the section comment, the `_main()` helper (its uses are confined to the doomed tests), and the nine seam tests — they pass `llm=_llm_cfg()` and would TypeError against M2's 4-arg signature. **Keep `_llm_cfg()`** (:519-521): the in-process endpoints section (:751/:765/:778/:791) uses it. The block's intent re-lands inverted in Slice 3's boundary test (`tests/test_no_double_route.py`: the pipeline must call llm_client zero times). Resulting region, byte-exact from `test_http_client_rejects_invalid_url` to the next surviving test (everything from :707 `def test_explicit_model_header_wins_over_env_chain` onward unchanged):
...
### tests/test_no_double_route.py — NEW (boundary contract test)
...
### .github/workflows/ci.yml:22-26 — MODIFY (pytest-asyncio on the CI install line)

The file is byte-identical in both forks (verified by diff), so the merge auto-merges it clean and this edit lands on the merged working tree. Exactly one line changes (:24):
...
### dashboard/src/App.jsx — MODIFY (take-theirs + 8-step manual resolution)

Base = their `dashboard/src/App.jsx` wholesale (`git checkout --theirs`, 2734 lines); every anchor below cites THEIR file. A1–A15 re-apply our deltas (import repoint, key lifecycle, gate, headers, targetClips, our mount deltas, reader patches, history-nav union). Their nav additions land untouched: the `voiceover`/`voice-style-presets`/`game-profiles` entries (:1127-1129), the `goToTab`/`tabLocked` tutorial lock (:1143-1148), the HistoryTab (:1969-1975) / GameProfilesPage (:1977) / VoiceOverPage mounted-once (:1987-2006) / VoiceStylePresetsPage (:2008-2014) view blocks, and the ClipTutorial modal (:2705-2713).
...
**A15.** History nav — D9 union rule. Replace their :1124-1126:
...
### dashboard/src/components/CreateEditProfileModal.jsx — MODIFY (props patch)

Base = their file (777 lines, arrives from the merge); every anchor cites THEIR file.
...
### dashboard/src/components/ResultCard.jsx — VERIFY (auto-merge carries both sides; no edit)

Slice-4 artifact correction: the skeleton had this as MODIFY (drop the dead `gemini_key` fallback). The merge tree (`a09ea6d`, re-derived in the Slice-4 session) shows git already lands BOTH sides: our dead-fallback removal sits at the merged file's :284-287 (comment + `const apiKey = geminiApiKey;` — our side changed that line after the base, theirs never touched it) AND their `onVoiceOver` prop (:39) + Mic voiceover button (:942-948) ride the auto-merge. ResultCard is not among the 7 textual conflicts, so no resolution — and no our-side delta — exists for it. No code is emitted; Slice 4's Success Criteria assert the merged state (grep gates: `getItem('gemini_key')` → 0 in this file, `onVoiceOver` present).

### dashboard/src/contexts/AuthContext.jsx — VERIFY (edit only if auto-merge drops a flag set)

No code is emitted: the merge tree — re-derived in both dashboard sessions (`a09ea6d` Slice 4, `b3ad3fd` Slice 5) and identical on every non-conflicted path — lands both sides clean. The pin, verbatim from the merged tree:
...
### docker-compose.yml — MODIFY (CPU flip + env_file required:false)
Auto-merged base = their compose whole (ours untouched since base — compose is NOT among the 7 conflicts); the D10 flip edits the merged tree, same shape as `ci.yml` in Slice 3. Five deltas vs their file: `build.args.GPU` dropped (Dockerfile `ARG GPU=0` is the real CPU default) with a 2-line opt-in comment; `env_file` → `required: false`; backend + renderer `gpus: all` deleted; renderer `NVIDIA_*` env removed (D10's inert-or-removed latitude — removed wins: the renderer NVENC path is a stub and a CPU-default file must not carry GPU-looking config); `TZ` + `hf-cache` kept. Full merged file:
...
### remotion/package.json — MODIFY (union manifest)
...
Resolution: take-theirs (`git checkout --theirs`) + our one-line zod delta (D11; the blanket D1 rule — their `2b83a64` bundled the zod drop with the animated-emoji add, and their own `render-service/package.json:18` pins zod `4.3.6` exact, so the union's `^4.3.6` cannot collide with their tree: their lock already resolves `zod@4.3.6` transitively, and the regen below changes only the root-deps block). Both deps are load-bearing in the merged src: `Subtitles.tsx:13` imports `@remotion/animated-emoji`, `lib/types.ts:1` imports zod — today they compile only as hoisted transitives.

Lockfile regeneration (commands, not code — machine-generated artifacts; per D11 both locks are committed because their `render-service/Dockerfile` COPYs both (:24, :31-32) and installs with `npm ci` (:25 plain, :38 `--legacy-peer-deps`)):

```bash
# On the merged tree (Slice 1's merge is still uncommitted):
...
### README.md — MODIFY (auto-merged tree b3ad3fd + 9 doc-claim rewrites)
...
R0. :302-303 — replace the clone command (developer decision this session: point at this fork's origin; the design didn't cover the auto-merged branding delta):
...
### Merge commit (Slice 8 finalize) — commands + message

The whole working tree is the merge resolution; the message lives OUTSIDE the repo so `git add -A` cannot stage it.
...
Deletions (no fences): `llm_backend.py` (DELETE, Slice 1), `tests/test_llm_backend.py` (DELETE, Slice 1). Regenerated artifacts (commands, not code): `remotion/package-lock.json`, `render-service/package-lock.json` (Slice 7).

## Slices

### Slice 1: Merge mechanics + app.py LLM seam (foundation)

**Files**: `app.py`, `llm_backend.py` (DELETE), `tests/test_llm_backend.py` (DELETE), `mcp_server.py`, `tests/test_llm_endpoints.py`

#### Automated Verification:
- [ ] `python -c "import app"` exits 0 — merged app.py imports clean with no llm_backend
- [ ] `grep -n "llm_backend" app.py mcp_server.py tests/test_llm_endpoints.py` → 0 matches
- [ ] `pytest tests/test_llm_endpoints.py tests/test_llm_client.py -q` green (ported block incl. `TestMergedGate`)
- [ ] `grep -c "ai_backend_available" app.py` >= 2 (definition + job-gate call site)
- [ ] `grep -c "resolve_llm" app.py` >= 8 (def + /api/llm/test task loop + /api/process + 5 satellite heads)
- [ ] `grep -n "await resolve_gemini" app.py` — every binding unpacks the 2-tuple; no raw-scalar binding survives

#### Manual Verification:
- [ ] `git checkout -b merge/marsic-parity && git merge --no-commit marsic/main` — merge in progress
- [ ] app.py conflict resolved per this slice; `git rm llm_backend.py tests/test_llm_backend.py` done
- [ ] remaining conflicts (main.py, dashboard/src/App.jsx, .env.example, CLAUDE.md, remotion/*) still open for their slices
- [ ] `pytest tests/test_llm_endpoints.py::TestMergedGate -q` green; POST /api/thumbnail/analyze with no keys → 400 LLM_ENDPOINT_HINT; /api/edit and /api/effects/generate gates fire on a missing key (tuple unpacks applied)

### Slice 2: main.py stage-ownership flip

**Files**: `main.py`, `tests/test_llm_client.py`

#### Automated Verification:
- [ ] `python -c "import main"` exits 0 — no `llm_backend` import remains; `ai_provider`/`hook_grounding`/`clip_quality` import clean
- [ ] `grep -rn "llm_backend" *.py cloud/ tests/` → 0 matches (main.py held the last six tokens — :29, :1569, :1574, :1583, :1937, :2921; Slice 1 cleared the rest)
- [ ] `grep -n "llm_client" main.py` → exactly 2 lines, both inside `get_visual_clips`' no-key branch (function-local import + gate call)
- [ ] `grep -n "score_batch_size\|LLM_SCORE_BATCH" main.py` → 0 matches
- [ ] `pytest tests/test_gemini_retry.py tests/test_gemini_block_split.py -q` green — the chain is 4-arg pure-Gemini again (merge-base shape)
- [ ] `pytest tests/test_llm_client.py tests/test_llm_endpoints.py -q` green — pipeline-branch block deleted, `_llm_cfg()` retained for the in-process endpoints section

#### Manual Verification:
- [ ] main.py resolved take-theirs (`git checkout --theirs main.py`) then M1-M4 applied; other conflicts untouched (`dashboard/src/App.jsx`, `.env.example`, `CLAUDE.md`, `remotion/*` stay open for their slices)
- [ ] `grep -n "def _run_gemini_stage" main.py` shows the 4-arg signature; `_run_stage_split` untouched from their side (8-arg, no `llm`)
- [ ] Persona (LLM_* triple set, no GEMINI_API_KEY): silent-video path prints the "text-only LLM server cannot see the footage" message and returns None; with no LLM_* either, our longer "cannot replace it" message. `get_viral_clips` never reads LLM_* (inherited from their fork: satellites dispatch via llm_client, the pipeline needs GEMINI or OPENAI_*)

### Slice 3: Boundary contract test + CI pytest-asyncio

**Files**: `tests/test_no_double_route.py` (NEW), `.github/workflows/ci.yml` (MODIFY, :24 only — the skeleton's "ci.yml" was the placeholder path; the file lives in `.github/workflows/` and is byte-identical in both forks)

#### Automated Verification:
- [ ] `pytest tests/test_no_double_route.py -q` green (6 tests: 2 video + 3 satellite + 1 app-layer)
- [ ] `grep -c "llm_backend" tests/test_no_double_route.py` returns 0 — the file must not reintroduce the deleted-module token (keeps Slice 2's repo-wide grep gate green)
- [ ] `pip install pytest-asyncio` then `pytest tests/test_game_profiles.py -q` green — their async tests run byte-identical
- [ ] `grep -n "pytest-asyncio" .github/workflows/ci.yml` returns exactly 1 match
- [ ] `pytest tests/test_llm_client.py tests/test_llm_endpoints.py -q` green — the new module's app import and env fixtures do not disturb the existing suites

#### Manual Verification:
- [ ] `git diff marsic/main -- tests/test_game_profiles.py` empty on the merge tree — their async tests byte-identical
- [ ] The CI workflow is otherwise untouched (no pinning-style change, no reordering)
- [ ] The new file names the deleted module in no comment or docstring (machine-checked above; reviewers keep it that way)

### Slice 4: Dashboard core seam

**Files**: `dashboard/src/App.jsx`, `dashboard/src/components/CreateEditProfileModal.jsx`, `dashboard/src/components/ResultCard.jsx`

#### Automated Verification:
- [ ] `cd dashboard && npm run build` exit 0 — merged App.jsx + patched CreateEditProfileModal compile against the merged AuthContext (both key families) and lib surface
- [ ] `cd dashboard && npm run lint` exit 0
- [ ] `grep -rn "lib/analytics" dashboard/src` → 0 matches (import repoint; research rename gate)
- [ ] `grep -n "setItem('gemini_key'" dashboard/src/App.jsx dashboard/src/components/*.jsx` → 0 matches
- [ ] `grep -n "getItem('gemini_key')" dashboard/src/App.jsx` → exactly 1 match (the one-way migration adopt line); same grep in `dashboard/src/components/ResultCard.jsx` → 0 matches
- [ ] `grep -c "llmHeaders(providerCfg)" dashboard/src/App.jsx` → 1
- [ ] `grep -c "needsAiBackend" dashboard/src/App.jsx` → >= 5 (gate, badge, banner, modal title, AI block)
- [ ] `grep -n "getItem('ai_provider')" dashboard/src/components/CreateEditProfileModal.jsx` → 0 matches; `grep -c "geminiApiKey" dashboard/src/components/CreateEditProfileModal.jsx` → >= 3
- [ ] `grep -c "llmConfig={providerCfg}" dashboard/src/App.jsx` → 2 (SaaShortsTab + ThumbnailStudio)
...
### Slice 5: Dashboard nav/mounts + AuthContext pin + deps

**Files**: `dashboard/src/App.jsx`, `dashboard/src/contexts/AuthContext.jsx` (verify), `dashboard/package.json`

#### Automated Verification:
- [ ] `cd dashboard && npm install && npm run build` exit 0 — merged App.jsx (A1-A15), merged AuthContext, union deps compile
- [ ] `cd dashboard && npm run lint` exit 0
- [ ] `grep -c "(!billingEnabled || isSignedIn)" dashboard/src/App.jsx` returns 1 (A15 union rule; neither fork's base contains the string)
- [ ] `grep -c "id: 'history'" dashboard/src/App.jsx` returns 1 and `grep -cE "id: '(voiceover|voice-style-presets|game-profiles)'" dashboard/src/App.jsx` returns 3 (their tabs land intact)
- [ ] `grep -c "tutorialLock && id !== 'dashboard'" dashboard/src/App.jsx` returns 2 (goToTab guard + tabLocked)
- [ ] `grep -c "voiceoverMounted &&" dashboard/src/App.jsx` returns 1; `grep -c "<HistoryTab onReopenProject" dashboard/src/App.jsx` returns 1; `grep -c "<GameProfilesPage" dashboard/src/App.jsx` returns 1; `grep -c "<VoiceStylePresetsPage" dashboard/src/App.jsx` returns 1
- [ ] `grep -c "localLlm" dashboard/src/contexts/AuthContext.jsx` returns >= 2; `grep -c "llmConfigured" dashboard/src/contexts/AuthContext.jsx` returns >= 2; `grep -c "from '../lib/panel'" dashboard/src/contexts/AuthContext.jsx` returns 1 (the auto-merge pin)
- [ ] `grep -c '"zod": "^4.3.6"' dashboard/package.json` returns 1; `grep -c '"@remotion/animated-emoji"' dashboard/package.json` returns 1; `git ls-files dashboard/package-lock.json` returns empty (their `72346de` deletion applied — tracked state; `npm install` may regenerate an untracked copy locally, which stays untracked); `grep -c '"zod": "4.3.6"' render-service/package.json` returns 1 (verify-only, their exact pin)
...
- [ ] `npm ls zod` inside dashboard resolves 4.x (no ^3 downgrade pulled by a stray peer range)

### Slice 6: Infra — compose flip + env union + requirements verify

**Files**: `docker-compose.yml`, `.env.example`, `requirements.txt` (verify), `Dockerfile` (verify — Files line amended at slice generation; File Map row added in lockstep, Slice-2 precedent)

#### Automated Verification:
- [ ] `docker compose config` exits 0 on the merged tree — CPU default, no `gpus: all`
- [ ] `grep -c "gpus: all" docker-compose.yml` returns 0
- [ ] `grep -c "NVIDIA_VISIBLE_DEVICES\|NVIDIA_DRIVER_CAPABILITIES" docker-compose.yml` returns 0
- [ ] `grep -c "args:" docker-compose.yml` returns 0 (GPU build-arg block dropped); `grep -c "required: false" docker-compose.yml` returns 1; `grep -c "hf-cache" docker-compose.yml` returns 2; `grep -c "TZ=Europe/Rome" docker-compose.yml` returns 1
- [ ] `grep -cE "\bAI_API_KEY\b|LLM_SCORE_BATCH|LLM_TIMEOUT|OLLAMA_CONTEXT_LENGTH|LLM_PROVIDER|GAME_PROFILE_ID|USER_ID" .env.example` returns 0 (moment-picker block dropped, never-documented list enforced; `\b` keeps `OPENAI_API_KEY`/`DEEP_OPENAI_API_KEY` from matching `AI_API_KEY`)
- [ ] `grep -c "LLM_MODEL_SAAS" .env.example` returns 1 (our satellite family documented; their file never had it); `grep -c "^# GEMINI_API_KEY=" .env.example` returns 1 (newly documented, both forks omitted it before)
- [ ] `grep -c "AI_TIMEOUT" .env.example` returns 1; `grep -c "DEEP_OPENAI_BASE_URL" .env.example` returns 1; `grep -c "ENABLE_CAPTION_EMOJIS" .env.example` returns 1; `grep -c "^# TARGET_CLIPS=" .env.example` returns 1; `grep -c "VOICEOVER_PRESETS_DIR" .env.example` returns 1; `grep -c "PAID_PROXY_DAILY_MB" .env.example` returns 1; `grep -c "HOOK_GROUNDING_WIDTH" .env.example` returns 1
- [ ] `git diff marsic/main -- requirements.txt Dockerfile` is empty (verify rows — their side lands byte-identical, this slice emits no edit for either file)
- [ ] `grep -c "openai==3.3.0" requirements.txt` returns 1; `grep -c "ARG GPU=0" Dockerfile` returns 2 (the CPU default's load-bearing half, Dockerfile :30/:63)
...
- [ ] Every var in the five new sections spot-greps to a real reader (AI_TIMEOUT/main.py, HOOK_GROUNDING_FRAMES/hook_grounding.py, TTS_DEVICE/voiceover.py, ENABLE_DEEP_ANALYSIS/main.py, ENABLE_VISION_ANALYSIS/main.py)
- [ ] The GPU opt-in remains discoverable: the compose build comment names --build-arg GPU=1; Dockerfile :22 documents the GPU build
- [ ] A keyless-LLM_BASE_URL user is pointed at the AI_PROVIDER=openai channel by the LLM section note (D14)

### Slice 7: Remotion union manifest + lockfiles
...
#### Automated Verification:
- [ ] Conflict resolved take-theirs + union manifest written; `grep -cF '"zod": "^4.3.6"' remotion/package.json` returns 1 and `grep -cF '"@remotion/animated-emoji": "4.0.447"' remotion/package.json` returns 1 (D11 union: their emoji pin kept, our zod range restored)
- [ ] `cd remotion && npm install --legacy-peer-deps` exit 0, then `cd remotion && npm ci --legacy-peer-deps` exit 0 — the regenerated lock is in sync with the union manifest (npm ci hard-fails on any manifest/lock drift); flag parity with their `render-service/Dockerfile:38`
- [ ] `cd render-service && npm ci` exit 0 — plain flag, their `render-service/Dockerfile:25` parity; their lock arrives in sync (root deps match their manifest exactly — the regen is a verified no-op)
- [ ] `cd remotion && npm run build` exit 0 — `tsc --noEmit` over the merged src (their `Subtitles.tsx:13` animated-emoji import + `remotion/src/lib/types.ts:1` zod import compile against declared deps, not transitive hoists)
- [ ] `git ls-files remotion/package-lock.json render-service/package-lock.json` returns exactly 2 lines — both committed (their Dockerfile :24/:32 COPYs them; a missing lockfile breaks the image build)
- [ ] `test -f remotion/public/fonts/Anton-Regular.ttf` — their font arrives via auto-merge; tsc will not catch its absence
- [ ] `git diff marsic/main -- render-service/package.json render-service/Dockerfile render-service/src` is empty — verify rows land byte-identical theirs
- [ ] `git status --porcelain remotion/ render-service/` shows no AA/UU entries after this slice
...
### Slice 8: Docs + finalize

**Files**: `CLAUDE.md`, `README.md`

#### Automated Verification:
- [ ] `git grep -n "llm_backend" -- "*.py" "*.md" ":!.rpiv"` → 0 matches (docs now token-free like the code)
- [ ] `grep -nE "LLM_SCORE_BATCH|OLLAMA_CONTEXT_LENGTH|LLM_TIMEOUT|LLM_PROVIDER|score_batch|llama3\.1|_run_gemini_stage|bisect" README.md CLAUDE.md` → 0 matches
- [ ] `grep -c "ai_backend_available" CLAUDE.md` returns >= 1; `grep -c "Stage ownership" CLAUDE.md` returns 1; `grep -c "owns the stage" CLAUDE.md` returns 1 (the standing rule, FRD req #9); `grep -n "phase 1" CLAUDE.md` → 0 matches
- [ ] `grep -c "AI_PROVIDER" README.md` returns >= 3; `grep -c "ukind/openshorts" README.md` returns 1; `grep -c "Marsic1/openshorts.git" README.md` returns 0
- [ ] `pytest -q` exit 0 — full suite on the merged tree (terminal-slice baseline; their 10 new files need pytest-asyncio from Slice 3)
- [ ] `cd dashboard && npm run build && npm run lint` exit 0
- [ ] `cd remotion && npm run build` exit 0; `test -f remotion/public/fonts/Anton-Regular.ttf`
- [ ] `docker compose config` exit 0
- [ ] Single merge commit: `git status --porcelain` empty after the commit; `git rev-list --no-merges --first-parent HEAD~1..HEAD` returns nothing (docs land inside the merge, no side commits on the first-parent line)
- [ ] `git merge-base --is-ancestor marsic/main HEAD` exits 0 — their 41 commits contained
- [ ] `git log -1 --format=%B | grep -c "^- "` returns 7 (per-file policy list, one bullet per conflicted file)

#### Manual Verification:
- [ ] Merge message reviewed end-to-end: 7 conflict policies + deletions + CI + compose + keyless note all present
- [ ] README link at the "Runs fully local" bullet resolves (`#6-run-without-a-google-key-local-llm-optional` heading unchanged)
- [ ] CLAUDE.md boundary section read against the stage-ownership map (D5/D2/D12/D14/D15)
- [ ] Accepted tension: base "Cómo se elige el layout" still says the layout picker calls Gemini; the Third-party LLM endpoint section below it states the reroute — base corpus is take-theirs, only their local-LLM section was rewritten (D1)
- [ ] Post-commit: `git log --oneline -3` shows the merge on `merge/marsic-parity`; `main` untouched (fast-forward is validate's job)
...
```
app.py                                        # MODIFY — their base + resolvers/predicate/config/gate/env-forward/satellite heads; import llm_backend deleted
llm_backend.py                                # DELETE — production-dead third LLM system
tests/test_llm_backend.py                     # DELETE — tests the deleted module (gate/config tests ported)
mcp_server.py                                 # MODIFY — _FORWARD_HEADERS +16 headers
tests/test_llm_endpoints.py                   # MODIFY — _clean_slate +7 env vars, gate/config port block
main.py                                       # MODIFY — their base; import llm_backend deleted; their dead local-LLM arm deleted (merge-base _run_gemini_stage restore); our seam NOT re-applied; score_batch_size deleted; silent-video gate → llm_client (function-local import)
tests/test_llm_client.py                     # MODIFY — pipeline-branch block deleted (9 seam tests + _main); _llm_cfg kept; intent re-lands inverted in the Slice 3 boundary test
tests/test_no_double_route.py                 # NEW — boundary contract (video side + satellite side + app-layer env)
.github/workflows/ci.yml                     # MODIFY — pytest-asyncio on the pip install line (:24; file byte-identical both forks)
dashboard/src/App.jsx                         # MODIFY — 8-step manual resolution (import repoint, key lifecycle, gate, headers, targetClips, nav, mounts, tutorial)
dashboard/src/components/CreateEditProfileModal.jsx  # MODIFY — key/provider via props; direct localStorage read removed
dashboard/src/components/ResultCard.jsx       # VERIFY — auto-merge lands our dead-fallback fix (:284-287) + their onVoiceOver (:39, :942-948); tree a09ea6d; no edit
dashboard/src/contexts/AuthContext.jsx        # VERIFY — auto-merge must carry llmConfigured/llmModel/llmBaseUrl AND localLlm; import from ../lib/panel
dashboard/package.json                        # MODIFY — + @remotion/animated-emoji; keep zod ^4.3.6 (reject their ^3.24.0 downgrade)
docker-compose.yml                            # MODIFY — CPU flip, env_file required:false, keep hf-cache
.env.example                                  # MODIFY — 9-section union (research §5); drop their moment-picker block
requirements.txt                              # VERIFY — openai==3.3.0 arrives via auto-merge
Dockerfile                                     # VERIFY — theirs lands whole (ARG GPU=0 default ×2, qwen-tts GPU pip block, conditional sox, fonts-symbola); no edit
remotion/package.json                         # MODIFY — union manifest (theirs + zod ^4.3.6)
remotion/package-lock.json                    # REGEN — npm install --legacy-peer-deps, committed
render-service/package-lock.json              # REGEN — same flag, committed
render-service/package.json                   # VERIFY — theirs (exact 4.0.447 pins)
CLAUDE.md                                     # MODIFY — their base + our corpus re-applied + boundary section
README.md                                     # MODIFY — their base (auto-merged); llm_backend doc sections rewritten
dashboard/src/lib/panel.js                    # VERIFY — survives (their comment edits auto-merge into it)
dev.bat / dev.sh                              # VERIFY — native dev scripts survive untouched
```

## Ordering Constraints

- Slice 1 **starts the merge**: `git checkout -b merge/marsic-parity && git merge --no-commit marsic/main`; conflicts for later slices stay unresolved until their slice. The tree stays uncommitted until Slice 8's single merge commit.
- Slice 2 requires Slice 1 (`llm_backend.py` deleted first; `main.py` import rewrite completes the module's removal).
- Slice 3 requires Slices 1+2 (boundary test needs the final dispatch map on both sides).
- Slice 4 requires Slice 1 (`/api/config` keys exist for the dashboard gate).
- Slice 5 requires Slice 4 (same file, sequential edits; fence rewritten with merged result).
- Slices 6-7 are independent of 1-5 in content but sequenced after so the env surface they document is final.
- Slice 8 last: docs describe the final state; the merge commit (message + policy list) is authored here.
- Auto-merged files are **verified, not edited** (File Map VERIFY rows) — except `AuthContext.jsx`, which is edited only if auto-merge dropped a flag set.
- After the merge commit and validation: `main` fast-forwards; the reference worktree is removed (`git worktree remove ../openshorts-marsic` — post-merge follow-up, outside this design).

## Verification Notes

Carried from research + FRD acceptance criteria; each maps to a slice:

- `grep -rn "llm_backend" *.py cloud/ tests/` → zero matches after Slice 2 (Slice 1 clears `app.py`/`mcp_server.py`/tests).
- Gate/config: `pytest tests/test_llm_endpoints.py -q` green incl. the ported block (Slice 1); `pytest tests/test_llm_client.py -q` stays green.
- No-double-route: `pytest tests/test_no_double_route.py -q` green (Slice 3).
- Full suites: `pytest -q` exit 0 including their 10 new files (needs pytest-asyncio, Slice 3).
- Dashboard: `cd dashboard && npm run build && npm run lint` exit 0 (Slice 5); `grep -rn "lib/analytics" dashboard/src` → 0 matches.
- Infra: `docker compose config` validates with CPU default and no `gpus: all` (Slice 6).
- Remotion: `cd remotion && npm ci --legacy-peer-deps` succeeds; `npm run build` (tsc --noEmit) passes; `remotion/public/fonts/Anton-Regular.ttf` present (tsc will NOT catch it — check the file) (Slice 7).
- Satellite spot-check: `thumbnail.py`/`saasshorts.py`/`layout_picker.py` still route via `llm_client` (grep `llm_client.chat` — unchanged files) (Slice 1/3).
- Persona e2e post-merge (validate stage): one job with Gemini only; one provider-only (`AI_PROVIDER=openai`); one no-keys job → clean 400, not a crash. Re-derive pinned counts (`grep -c 'startswith("LLM_")' app.py`) — namespaces grew.
- Merge commit: `git log --oneline marsic/main` fully contained in `merge/marsic-parity`; merge message carries the per-file policy list (Slice 8).
...
- Merged satellite gate pattern: ours `app.py:4891-4900` (adapted to their 2-tuple `resolve_gemini`, `../openshorts-marsic/app.py:138-150`).
- `describe()` dict shape asserted verbatim: their `tests/test_llm_backend.py:49-52`.
- Canary + counting-fake test patterns: ours `tests/test_llm_client.py:722-728` (`_no_genai`), `:744-745` (`sample_frames` stub).
- Env clean-slate fixture: ours `tests/test_llm_endpoints.py:25-30` (`_clean_slate`).
- Merge commit message + policy list: `0f326fd` (their upstream merge); house format `Merge <source>: <summary>` (`989e69c`, `a42297f`).
- Key-via-props component pattern: their `App.jsx:1990-2003` (`VoiceOverPage` mount).

## Developer Context

Inherited from research checkpoint (all fixed upstream — never re-asked): take-theirs-then-reapply; delete `llm_backend.py`; unified `ai_backend_available()`; full parity config-gated; stage-ownership boundary (advisor-hardened); merge commit on branch; CPU default; remotion union + committed lockfile (research correction); no permanent remote; their error semantics wholesale; compose `env_file` → `required: false`; keyless strictness accepted + documented; history nav union rule.

This session (design stage):

- **Q (directional, CLAUDE.md base)**: FRD blanket rule vs doc economics — our richer corpus vs their base? `file:line` evidence: our CLAUDE.md carries 15 commits of architecture docs; their `CLAUDE.md:140-151` is the rewritten-either-way section.
  **A: "Take theirs, re-apply ours"** — blanket rule holds for all 7 conflicted files.
- **Q (design summary confirm)**: approach + 7 stated decisions + scope + file counts presented.
  **A: Proceed.**
- **Q (decomposition confirm)**: 8 slices as listed in `## Slices`.
  **A: Approve.**

Slice 2 session (micro-checkpoint):

- **Q (Slice 2/8 approve)**: presented M1-M4 (main.py) + T1 (test_llm_client.py) with two research-table resolutions the File Map had compressed: (1) their `_run_gemini_stage` local-LLM arm **deleted, not rewired** — production-dead (chain reached only by `_run_stage_split` `../openshorts-marsic/main.py:1914`; live score/detail call `create_ai_provider` at :2528/:2783), and a rewired arm would plant an llm_client dispatch in pipeline code the Slice-3 boundary test canaries against; (2) **Files list gains `tests/test_llm_client.py`** — the research break-list mandates porting the :507-705 seam block out, and no slice carried the file.  **A: Approve (Recommended).


Slice 3 session (micro-checkpoint):

- **Q (Slice 3/8 approve)**: presented the 6-test boundary file (video 2 / satellite 3 / app-layer 1; counting fakes on the owned side, raising canaries on the other, both LLM families configured at once) plus the one-line CI install change, with two verified artifact corrections: their async-marked tests are 6, not 7 (`../openshorts-marsic/tests/test_game_profiles.py:20/:58/:81/:105/:140/:164`), and the CI file lives at `.github/workflows/ci.yml`, byte-identical in both forks (clean auto-merge). Verifier round 1 caught the transcript word format (their `main.py:1973` reads `word['word']/['start']/['end']`; the builder had emitted `{'w','s','e'}`); fixed the `_words()` keys. Round 2 clean; advisory recorded in Design History: never assert `end <= duration` for the whole-clip fake short (no clamp outside the repair branch).  **A: Approve (Recommended).
Slice 5 session (micro-checkpoint):

- **Q (Slice 5/8 approve)**: presented A15 (the D9 history-nav union spread — the only App.jsx code edit; their `voiceover`/`voice-style-presets`/`game-profiles` tabs, the `goToTab`/`tabLocked` tutorial lock, and the four view mounts land via take-theirs, verified not edited), the AuthContext verify pin (both merge-tree runs agree; both key families in one `value` object), and the `dashboard/package.json` zod `^4.3.6` revert with its evidence chain (v3/v4-neutral API at `dashboard/src/remotion/lib/types.ts:1` / `remotion/src/lib/types.ts:1` / `render-service/src/server.ts:3`, `render-service/package.json:18` exact pin, base `ad8ab59` carried `^4.3.6`).  **A: Approve (Recommended).**

Slice 8 session (micro-checkpoint):

- **Q (clone URL)**: their auto-merged README rewrites the clone command to `Marsic1/openshorts.git` (merged `:302-303`) while the badges two paragraphs up stay `mutonby`; this repo's origin is `ukind/openshorts` (`git remote -v`). README is NOT one of the 7 conflicts, so the design's D1 rule never covered this delta.  **A: "Point at ukind (our origin)"** — R0 records it; the merge note's closing line says "README clone URL points at this fork".
- **Q (Slice 8/8 approve)**: presented the CLAUDE.md boundary rewrite (their `:140-151` local-LLM section → endpoint + stage-ownership sections), the 9 README claim fixes (R0-R8, content-located), and the D16 merge commit (house subject + D14 note + 7-bullet per-file policy list); 2 verifier rounds (round 1: R7 anchor :608→:613-614, content-located R-blocks, message outside the repo, `--first-parent`; round 2 clean + grep floor =7).  **A: Approve (Recommended).**

## Design History

- Slice 1: Merge mechanics + app.py LLM seam — approved as generated (2 verifier rounds: BYOK test model header, token-free comments, 3 anchor-range fixes, E11 effects tuple unpacks added)
- Slice 2: main.py stage-ownership flip — approved as generated (2 verifier rounds: M4 anchor range fixed to :2919-2927, "none of our ec60f4f hunks" tightened to seam hunks, six-token count; Files list amended to carry tests/test_llm_client.py per the research break-list — the skeleton had omitted the carrier file)
- Slice 3: Boundary contract test + CI pytest-asyncio — approved as generated (2 verifier rounds: round 1 caught the transcript word format — their main.py:1973 reads word['word']/['start']/['end'] while the builder emitted {'w','s','e'}, both video tests would KeyError before any provider call; fixed. Round 2 clean with one advisory: the whole-clip detail fake short survives unclamped on the ≤120s path, so the test must never assert end <= duration. Artifact corrections: their async-marked tests are 6, not 7; the CI file's real path is .github/workflows/ci.yml, byte-identical in both forks)
- Slice 4: Dashboard core seam — approved as generated (2 verifier rounds: round 1 caught 18 anchor line-number drifts — every quoted block was verbatim-correct, the ranges had come from truncated reads; corrected to base-file line numbers, round 2 clean apart from a non-blocking warning on A7's compressed description, the fence retains all their header families. Artifact correction: ResultCard.jsx relabeled MODIFY → VERIFY — the merge tree a09ea6d shows the auto-merge already lands our dead-fallback fix (:284-287) alongside their onVoiceOver prop (:39, :942-948); no edit emitted, criteria assert the merged state)
- Slice 5: Dashboard nav/mounts + AuthContext pin + deps — approved as generated (2 verifier rounds: round 1 caught a Success-Criteria self-contradiction — `npm install` regenerates `dashboard/package-lock.json`, so the later `test ! -f` check fails when the list runs in order; fixed to `git ls-files` tracked state, which holds under both orderings. Round 2 clean. Evidence added: their zod surface is v3/v4-neutral and `render-service/package.json:18` pins `4.3.6` exact, so the `^3.24.0` downgrade (2b83a64, bundled with animated-emoji) is incidental and safely rejected; their `72346de` lockfile deletion auto-applies; merge trees `a09ea6d`/`b3ad3fd` agree on all non-conflicted paths — they differ only in conflict-marker noise inside the 7 conflicted files)
- Slice 6: Infra — compose flip + env union + requirements verify — approved as generated (2 verifier rounds: round 1 caught three grep-token collisions — the GPU opt-in comment contained the literal `gpus: all` it gated against, `AI_API_KEY` matched as a substring of `OPENAI_API_KEY`, `TARGET_CLIPS` double-counted from its own header — plus a missing File Map row for the amended Files list; fixed with a reworded comment, word-bounded/anchored greps, and the lockstep File Map row. Round 2 clean with one ordinal warning fixed in place: "five" new env sections, not four. Deviations recorded: our LLM section's stage list drops "clip scoring/detail" per D5/M1 — research said "verbatim", but the stage moved to ai_provider and the doc would lie; renderer NVIDIA_* removed under D10's inert-or-removed latitude; ai_provider.py:467/:470 default drift (gemini-2.5-flash/gpt-4) is a dead fallback — every create_ai_provider call passes explicit model_name — recorded as a post-merge follow-up, no slice owns that module)
- Slice 7: Remotion union manifest + lockfiles — approved as generated (2 verifier rounds: round 1 passed Decisions/Cross-slice and caught two research misattributions — the zod hoist requirers in their lock are @remotion/media + @remotion/studio, not @remotion/zod-types (peer-range only), and a second unflagged `cd remotion && npm ci` at Verification Notes; fixed, round 2 clean. Deviation recorded: the artifact's Desired End State and Verification Notes remotion bullets gained the `--legacy-peer-deps` flag for render-service/Dockerfile:38 parity — prose synced to D11, code unchanged)
- Slice 8: Docs + finalize — approved as generated (2 verifier rounds: round 1 caught four atomicity defects — the R7 retry anchor pointed at :608-609 instead of :613-614 (leaving `_run_gemini_stage` alive), the R-blocks cited one line numbering while R3/R4/R8 shift later anchors (now located by their unique quoted first line), `git add -A` would stage `MERGE_MSG.txt` into the merge (message now lives at `../MERGE_MSG.txt`, outside the repo), and the single-commit check needed `--first-parent` (the plain range counts the second parent's 40 non-merge commits); round 2 clean apart from the policy-list grep floor, raised to =7. Deviations recorded: our re-applied endpoint section drops "clips score/detail" from the reroute list and the stale "phase 1" tag — same D5 correction class as Slice 6's .env stage list; README clone URL points at `ukind/openshorts` per the Slice-8 checkpoint question (their `Marsic1` URL landed via the auto-merge and the design didn't cover it; badges stay `mutonby`); the 7th policy bullet (remotion lock) split out of the package.json bullet for the 1:1 per-file list)

## References

- Research: `.rpiv/artifacts/research/2026-09-06_06-09-59_marsic-fork-parity-merge.md` (per-hunk re-apply table, resolver inventory, port lists, test composition)
- FRD: `.rpiv/artifacts/discover/2026-09-06_05-40-55_marsic-fork-parity-merge.md` (11 requirements, acceptance criteria, decisions)
- Prior design: `.rpiv/artifacts/designs/2026-08-30_08-58-35_openai-compatible-llm-provider.md` (llm_client contract)
- Prior validation: `.rpiv/artifacts/validation/2026-09-05_15-07-32_connect-llm-provider-frontend.md` (personas, pinned counts)
- Their tree: `../openshorts-marsic` @ b1f5350 (read-only reference)
- Their upstream merge precedent: `0f326fd`; gate fixes: `56707b7` (ours), `25222f5` (theirs)
