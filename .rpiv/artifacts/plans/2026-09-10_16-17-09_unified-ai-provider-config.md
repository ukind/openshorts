---
date: 2026-09-10T16:17:09+0700
author: Yogiswara Utama
commit: e5ffe7c
branch: main
repository: openshorts
topic: "Unified AI provider config — single source of truth BYOK"
tags: [plan, ai-provider, llm-client, byok, frontend, editor, thumbnail, hook-grounding, screencast]
status: ready
parent: .rpiv/artifacts/designs/2026-09-10_07-09-56_unified-ai-provider-config.md
phase_count: 7
phases:
  - { n: 1, title: "Unified card + encrypted store + migration (FE core)", files: [dashboard/src/lib/llm.js, dashboard/src/components/AiProviderCard.jsx, dashboard/src/App.jsx, dashboard/src/components/LlmProviderCard.jsx, dashboard/src/contexts/AuthContext.jsx, app.py, tests/test_llm_endpoints.py], depends_on: [] }
  - { n: 2, title: "FE emission convergence", files: [dashboard/src/App.jsx, dashboard/src/components/ResultCard.jsx, dashboard/src/components/VoiceOverPage.jsx, dashboard/src/components/CreateEditProfileModal.jsx], depends_on: [1] }
  - { n: 3, title: "hook_grounding.py reroute", files: [hook_grounding.py, tests/test_hook_grounding.py, CLAUDE.md], depends_on: [] }
  - { n: 4, title: "screencast_layout.py reroute", files: [screencast_layout.py, tests/test_screencast_layout.py, CLAUDE.md], depends_on: [] }
  - { n: 5, title: "editor.py frames path + endpoint resolution", files: [editor.py, app.py, tests/test_editor_frames.py], depends_on: [] }
  - { n: 6, title: "thumbnail image reroute + gate relax", files: [thumbnail.py, app.py, tests/test_thumbnail_studio.py], depends_on: [5] }
  - { n: 7, title: "Pre-existing lint debt cleanup (repo-wide green)", files: [dashboard/src/Legal.jsx, dashboard/src/components/PricingPage.jsx, dashboard/src/components/RemotionPreview.jsx, dashboard/src/components/SubtitleModal.jsx, dashboard/src/components/VoiceOverPage.jsx, dashboard/src/components/VoiceStylePresetsPage.jsx], depends_on: [] }
last_updated: 2026-09-10T18:29:13+0700
last_updated_by: Claude
last_updated_note: "Revision 1: added Phase 7 (pre-existing lint debt cleanup, 32 errors + 3 warnings at HEAD e5ffe7c); Phase 1/Phase 2 lint criteria scoped until it lands."
---

# Unified AI provider config — single source of truth BYOK: Implementation Plan

## Overview

This plan implements the design "Unified AI provider config — single source of truth BYOK" (parent artifact below). One encrypted dashboard store (aiProviderConfig_v1) and one card (AiProviderCard.jsx) feed both backend config families: X-AI-Provider + X-OpenAI-* headers for the pipeline, derived X-LLM-* headers for the satellites. The single emitter is dashboard/src/lib/llm.js. Four Gemini-only stages gain OpenAI-compatible arms: hook grounding, screencast layout, editor effects, thumbnail image generation. Dispatch modules stay untouched. Cloud and billing behavior stays untouched. Phase boundaries inherit 1:1 from the design slices. Success Criteria pass through verbatim from the design.
...
## Phase 1: Unified card + encrypted store + migration (FE core)

### Overview
The one card and the one encrypted store. The legacy llmConfig_v1 and openai_* keys import once, then leave the browser. lib/llm.js emits both header families. /api/config reports the server openai* fields (FR9).

### Changes Required:

#### 1. dashboard/src/lib/llm.js
**File**: `dashboard/src/lib/llm.js`
**Changes**: The dual-family single emitter: `aiProviderSet` predicate (D10) + `openaiHeaders` builder alongside the existing `llmConfigComplete`/`llmHeaders`.
...
#### 2. dashboard/src/components/AiProviderCard.jsx
**File**: `dashboard/src/components/AiProviderCard.jsx`
**Changes**: The one card: form (baseUrl/apiKey/model), presets, Test connection (LLM test when keyed, models probe when keyless), model dropdown, server-config badge; saves trimmed to `onConfigSet`; keyless and empty-model are saveable (D10).
...
#### 4. dashboard/src/components/LlmProviderCard.jsx
**File**: `dashboard/src/components/LlmProviderCard.jsx`
**Changes**: Deleted outright (D7). Its affordances were re-homed into AiProviderCard.jsx: presets, the /api/llm/test connection flow, the derived server badge, the show/hide key toggle.
...
#### 5. dashboard/src/contexts/AuthContext.jsx
**File**: `dashboard/src/contexts/AuthContext.jsx`
**Changes**: FR9 exposure: three consts (`openaiConfigured`, `openaiModel`, `openaiBaseUrl`) derived from the `/api/config` fetch, added to the context `value` object next to the `llm*` trio.
...
#### 6. app.py
**File**: `app.py`
**Changes**: Slice 1 portion only: /api/config gains three read-only, billing-pinned openai* fields (FR9), next to the existing llm* fields. Slices 5 and 6 touch other regions of this file (see Phases 5 and 6).
...
#### 7. tests/test_llm_endpoints.py
**File**: `tests/test_llm_endpoints.py`
**Changes**: One additive `TestConfigFields` test pinning the new `/api/config` openai* fields (env set → true/model/base). All 26 existing tests untouched.
...
- [x] `cd dashboard && npx eslint src/App.jsx src/lib/llm.js src/components/AiProviderCard.jsx src/contexts/AuthContext.jsx --report-unused-disable-directives --max-warnings 0` passes — scoped to the phase's own files (verified exit 0 post-implementation). Repo-wide green is pre-existing debt owned by Phase 7, not this phase.
- [x] `cd dashboard && npm run build` passes
- [x] `grep -r "LlmProviderCard" dashboard/src` returns no matches
- [x] `grep -n "setItem('openai_" dashboard/src/App.jsx` returns no matches (plaintext writers gone; the only remaining openai_* literals are the migration reads inside the store initializer)
- [x] `grep -nE "headers\['X-OpenAI-" dashboard/src/App.jsx` returns no matches (pipeline emission only via lib/llm.js openaiHeaders)
- [x] `pytest tests/test_llm_endpoints.py -q` passes, including the additive `test_config_reports_server_openai_env`; all 26 existing tests unmodified and green
- [x] `pytest tests/test_no_double_route.py -q` passes (env contract untouched)
- [x] `pytest tests/test_llm_client.py -q` passes — file untouched; the 56 tests pin the derived X-LLM-* emission (added at plan Step 5 triage)
...
- [ ] Card saved with base+key and model empty: a satellite request (e.g. thumbnail titles) sends no `X-LLM-Model` and the server resolves `LLM_MODEL_<TASK>`/`LLM_MODEL` (added at plan Step 5 triage)

---

## Phase 2: FE emission convergence

### Overview
Every frontend call site emits through the lib/llm.js builders. The transitional prop shims from Phase 1 fold into one unified prop. The plaintext key leaves the game-profile bodies (D11).

### Changes Required:

#### 1. dashboard/src/App.jsx
**File**: `dashboard/src/App.jsx`
**Changes**: Section I final state only: replace the transitional prop shims (installed in Phase 1 at the VoiceOverPage and CreateEditProfileModal mounts) with the unified prop, and add the same prop to the ResultCard mount after elevenLabsKey. The aiProvider={aiProvider} prop lines stay untouched at both mounts.
...
#### 2. dashboard/src/components/ResultCard.jsx
**File**: `dashboard/src/components/ResultCard.jsx`
**Changes**: `handleAutoEdit` gains both derived families on `/api/effects/generate` + `/api/edit` (today sends only `X-Gemini-Key`, `ResultCard.jsx:294`); the "Gemini API Key is missing" pre-check relaxes to "no AI backend at all".
...
#### 3. dashboard/src/components/VoiceOverPage.jsx
**File**: `dashboard/src/components/VoiceOverPage.jsx`
**Changes**: `byokHeaders()` (`:346-354`) folds into the shared builders; the unified prop replaces `openaiApiKey/Model/BaseUrl` props.
...
#### 4. dashboard/src/components/CreateEditProfileModal.jsx
**File**: `dashboard/src/components/CreateEditProfileModal.jsx`
**Changes**: `handleAiAnalyze` inline builder (`:253-264`) folds into the shared builders; profile save drops `openai_key` from the body (D11); unified prop replaces the triple.
...
[Showing lines 1-758 of 3153 (50.0KB limit). Use offset=759 to continue.]
...
#### Automated Verification:
- [x] `cd dashboard && npx eslint src/App.jsx src/components/ResultCard.jsx src/components/VoiceOverPage.jsx src/components/CreateEditProfileModal.jsx --report-unused-disable-directives --max-warnings 0` passes — scoped to the phase's own files (the repo-wide lint gate lands with Phase 7) (zero new lint errors from this phase; the exit-1 set is Phase 7's pre-existing VoiceOverPage.jsx :40/:233 findings, identical at HEAD)
- [x] `cd dashboard && npm run build` passes
- [x] `grep -rn "openaiApiKey" dashboard/src` returns no matches (the dead prop name is fully gone)
- [x] `grep -rn "openai_key" dashboard/src/components/CreateEditProfileModal.jsx` returns no matches (D11 — the key never rides the body)
- [x] `grep -rn "headers\['X-OpenAI-\|headers\['X-LLM-" dashboard/src | grep -v "lib/llm.js"` returns no matches (emission-form literals only in the single emitter)
- [x] `grep -c "openaiHeaders" dashboard/src/components/ResultCard.jsx dashboard/src/components/VoiceOverPage.jsx dashboard/src/components/CreateEditProfileModal.jsx` returns >= 1 for each file
- [x] `pytest tests/test_no_double_route.py -q` passes (FE-only slice; dispatch contract untouched)
...
## Phase 3: hook_grounding.py reroute

### Overview
hook_grounding.py gains an OpenAI arm that reads the job OPENAI_* env, guarded by probe_vision_support. The arm degrades with one log line. It never fails the clip.

### Changes Required:

#### 1. hook_grounding.py
**File**: `hook_grounding.py`
**Changes**: OpenAI arm inside `reground`: when `GEMINI_API_KEY` is absent, read the job `OPENAI_*` env, build via `ai_provider.create_ai_provider`, guard with `probe_vision_support`, call with frames + `GroundedHook` schema; `_ask_openai` seam beside `_ask_gemini`; skip semantics unchanged.
...
#### 2. tests/test_hook_grounding.py
**File**: `tests/test_hook_grounding.py`
**Changes**: New stubs/tests for the OpenAI arm: skip without vision after probe `no_vision`; skip with no provider at all; reground succeeds through the OpenAI stub; `_ask_gemini` seam preserved.
...
#### 3. CLAUDE.md
**File**: `CLAUDE.md`
**Changes**: One paragraph: the "Hook grounding for on-screen clips" section no longer claims Gemini-only. The manual criterion covers the wording.

### Success Criteria:

#### Automated Verification:
- [x] `pytest tests/test_hook_grounding.py -q` passes — all 12 existing tests (one hardened with OPENAI_* delenv) plus the 8 new OpenAI-arm tests
- [x] `pytest tests/test_no_double_route.py -q` passes, the file byte-unmodified (the reroute canary lives in `tests/test_hook_grounding.py::test_openai_arm_never_touches_the_satellite_client`)
- [x] `pytest tests/test_ai_provider.py -q` passes (ai_provider.py untouched)
- [x] `grep -n "llm_client\|LLM_BASE_URL\|LLM_API_KEY\|LLM_MODEL" hook_grounding.py` returns no matches — env namespace law: the arm reads job `OPENAI_*` only
...
## Phase 4: screencast_layout.py reroute

### Overview
detect_content_ranges in screencast_layout.py gains a frames arm. The gate and degrade semantics match Phase 3. The Gemini Files path stays first.

### Changes Required:

#### 1. screencast_layout.py
**File**: `screencast_layout.py`
**Changes**: `detect_content_ranges` gains a frames arm: when Gemini is absent but the job `OPENAI_*` env resolves, sample 12 frames @1024px (`layout_picker.sample_frames`), call via `ai_provider` with `WideContentResponse` schema + probe guard; degrade-to-`[]` unchanged.
...
#### 2. tests/test_screencast_layout.py
**File**: `tests/test_screencast_layout.py`
**Changes**: Frames path returns ranges through an OpenAI-compat stub; degrade to `[]` on provider failure; no-provider stays silent.
...
#### 3. CLAUDE.md
**File**: `CLAUDE.md`
**Changes**: One paragraph: the "SCREENCAST / WIDE Modes" bullet no longer credits only Gemini.

### Success Criteria:

#### Automated Verification:
- [x] `pytest tests/test_screencast_layout.py -q` passes — all 19 existing tests unmodified plus the 10 new OpenAI-arm tests
- [x] `pytest tests/test_no_double_route.py -q` passes, the file byte-unmodified (the reroute canary lives in `tests/test_screencast_layout.py::test_openai_arm_never_touches_the_satellite_client`)
- [x] `pytest tests/test_ai_provider.py -q` passes (ai_provider.py untouched)
- [x] `pytest tests/test_hook_grounding.py -q` passes (sibling in-job arm untouched by this slice)
- [x] `grep -n "llm_client\|LLM_BASE_URL\|LLM_API_KEY\|LLM_MODEL" screencast_layout.py` returns no matches — env namespace law: the arm reads job `OPENAI_*` only
...
## Phase 5: editor.py frames path + endpoint resolution

### Overview
editor.py gains a frames analysis path. /api/edit and /api/effects/generate resolve the OpenAI triple in-request and pick the arm. Shared prompts and a shared renderer keep the Gemini path unchanged.

### Changes Required:

#### 1. editor.py
**File**: `editor.py`
**Changes**: Frames-based analysis path: module-level function(s) building a provider from an OpenAI triple, sampling frames, asking for `EditPlan`/effects JSON with probe guard; `VideoEditor` Gemini path unchanged; `apply_edits` shared.
...
[Showing lines 761-1729 of 3153 (50.0KB limit). Use offset=1730 to continue.]
...
#### 2. app.py
**File**: `app.py`
**Changes**: Slice 5 portion: the /api/edit and /api/effects/generate key blocks. Resolve the OpenAI triple in-request (caption precedent app.py:6786-6807), branch the Gemini Files path against the frames path, and raise a clean error when neither backend exists.
...
#### 3. tests/test_editor_frames.py
**File**: `tests/test_editor_frames.py`
**Changes**: Mode selection (Gemini key → Files; absent + OpenAI → frames); EditPlan JSON on a compat stub; text-only filter repair; clean 4xx when no backend at all.
...
#### Automated Verification:
- [x] `pytest tests/test_editor_frames.py -q` passes — all new tests green (gate predicate, vision arm, both frames arms, repair routing, the no-double-route canary, both endpoint gates)
- [x] `pytest tests/test_no_double_route.py -q` passes, the file byte-unmodified (the canary lives in `tests/test_editor_frames.py::test_the_arm_never_touches_the_satellite_client`)
- [x] `pytest tests/test_ai_provider.py -q` passes (ai_provider.py untouched)
- [x] `pytest tests/test_edit_builder.py -q` passes (the build_filter_string call contract is unchanged)
- [x] `pytest tests/test_llm_endpoints.py -q` passes (the existing endpoint surface is untouched)
- [x] `grep -n "llm_client\|LLM_BASE_URL\|LLM_API_KEY\|LLM_MODEL" editor.py` returns no matches — env namespace law: the arm reads the resolved OPENAI_* triple only
- [x] `grep -c "openai_provider=openai_prov" app.py` returns 2 (both editor constructions carry the arm)
...
## Phase 6: thumbnail image reroute + gate relax

### Overview
thumbnail.py gains a /v1/images/generations arm (text-prompt only, D14). The gate accepts the unified config without a Gemini key. Cloud behavior stays byte-identical.

### Changes Required:

#### 1. thumbnail.py
**File**: `thumbnail.py`
**Changes**: Image reroute: `/v1/images/generations` arm beside the `genai` call (text-prompt only, D14); Gemini fallback; clean-unavailable state when the endpoint lacks image support; reference images Gemini-arm only.
...
#### 2. app.py
**File**: `app.py`
**Changes**: Slice 6 portion: the /api/thumbnail/generate gate accepts the unified config. The billing branch (:6058+) stays untouched.
...
#### 3. tests/test_thumbnail_studio.py
**File**: `tests/test_thumbnail_studio.py`
**Changes**: `/v1/images/generations` success; refusal/absence falls back to Gemini or yields the clean-unavailable state; gate accepts unified config with no Gemini key (billing branch untouched).
...
[Showing lines 1732-2695 of 3153 (50.0KB limit). Use offset=2696 to continue.]
...
#### Automated Verification:
- [x] `pytest tests/test_thumbnail_studio.py -q` passes — all 8 existing tests byte-unmodified plus the 14 new image-arm tests (10 module + 4 gate)
- [x] `pytest tests/test_no_double_route.py -q` passes, the file byte-unmodified (the canary lives in `tests/test_thumbnail_studio.py::test_the_images_arm_never_touches_the_satellite_client`)
- [x] `pytest tests/test_editor_frames.py -q` passes (the shared `openai_configured` predicate is reused, not redefined — `grep -n "def openai_configured" *.py` still returns only `editor.py`)
- [x] `pytest tests/test_llm_endpoints.py -q` passes (the existing endpoint surface is untouched)
- [x] `grep -n "LLM_BASE_URL\|LLM_API_KEY\|LLM_MODEL" thumbnail.py` returns no matches — env namespace law: the images arm's config arrives as the resolved `OPENAI_*` triple from `resolve_openai`, never `LLM_*` env (the concept-text `llm_client` delegation is unchanged and does its own env reads)
...
- [x] `git diff --quiet -- llm_client.py cloud/alerts.py` exits 0 — the "LLM provider" error prefixes stay byte-identical, `cloud/alerts.py` classifies on them (added at plan Step 5 triage)
...
#### Reconciliation
- `app.py`: replace `    # precedence as the in-job stages. openai_configured rejects resolve_openai's` → `    # precedence as the in-job stages. the gate predicate rejects resolve_openai's` — Phase 6 AV `grep -c "openai_configured" app.py` expects 4 (one import + the three gate call lines); the landed Phase 5 comment line at :3499 is a fifth matching line because grep -c counts comment lines too, and this Phase 6 lane may not edit a sibling phase's region. The mechanical reword brings the count to 4; behavior untouched.
- [ ] Cloud mode: `resolve_llm` still returns `None` (`app.py:193-194` unmodified) — satellite calls keep the managed path (added at plan Step 5 triage)

---

## Phase 7: Pre-existing lint debt cleanup (repo-wide green)

### Overview

`npm run lint` at HEAD `e5ffe7c` fails on 32 errors + 3 warnings across six dashboard files that no other phase of this plan touches (measured 2026-09-10, post-Phase-1 implementation run; the files are byte-identical to HEAD, so the failures pre-date the plan). This phase owns that debt so the repo-wide strict gate can pass. `depends_on: []` — in practice run it right after Phase 1, so Phase 2's scoped lint criterion is a stepping stone toward the global green rather than a permanent exception.

Behavior preservation is the hard rule: this phase deletes dead code and never silences findings with blanket disables. Line numbers below come from the measured inventory; an added `eslint-disable` is allowed only where it suppresses a real, reasoned exception — a disable that suppresses nothing fails under `--report-unused-disable-directives`.

### Changes Required:

#### 1. dashboard/src/components/SubtitleModal.jsx — 23 findings, the bulk
**File**: `dashboard/src/components/SubtitleModal.jsx`
**Changes**:
- 12 empty block statements — `:141:17`, `:159:98`, `:166:98`, `:222:98`, `:254:17`, `:305:59`, `:356:104`, `:700:79`, `:724:114`, `:775:114`, `:783:99`, `:833:69`. Empty `catch`/callback bodies: each gets a one-word comment inside the braces (`/* ignored */`) or outright deletion of the dead handler — judgment per site, no behavior change either way (the block is already empty).
- 10 dead locals — `last` (`:443:21`), `buildForGap` (`:490:15`), `gapLen` (`:514:27`), `prevOiEnd` (`:530:13`), `prevNjEnd` (`:531:13`), `gapIdx` (`:554:13`), `anchorIdx` (`:555:13`), `anchorMs` (`:599:85`), `charIdx` (`:637:13`) — leftovers of a removed gap-anchoring algorithm. DELETE each binding together with every assignment to it and any now-orphaned comment; do not underscore-prefix — the names document a mechanism that no longer exists.
- `:445:81` `no-misleading-character-class`: a regex character class containing a combining character. Rewrite the class with explicit escapes (or drop the combining sequence if it duplicates a precomposed one); the regex must still compile and match the same inputs.
- 2 `react-hooks/exhaustive-deps` warnings — `:788:8` (`handleTextEdit`), `:875:8` (`updateWordTiming`). Move the named function where the linter wants it, or add the dep — whichever preserves the current call timing. Do not silence with a disable.

#### 2. dashboard/src/components/VoiceStylePresetsPage.jsx — 5 findings
**File**: `dashboard/src/components/VoiceStylePresetsPage.jsx`
**Changes**:
- `:1:55` unused `useCallback` import — remove from the import list.
- `:36:9` dead `suggestions` local — inspect first: if no reader exists, delete the binding and its writers as a pair.
- `:229:35` / `:499:35` unused args `Icon` / `TabIcon` — underscore-prefix (`_Icon`, `_TabIcon`) if callers still pass them; remove from the signature otherwise.
- `:459:20` `no-constant-binary-expression` — a constant-truthy left side of `&&`: a dead branch. Inspect, then delete the dead side or correct the constant to the real condition (a real bug fix — name it in the commit message).
- `:253:32` `exhaustive-deps` on `load` — add `load` to the deps; hoist it into a `useCallback` if adding the raw dep would regress run counts.

#### 3. dashboard/src/components/VoiceOverPage.jsx — 2 findings (Phase 2's file; both sites sit OUTSIDE Phase 2's edit regions `:84-86`/`:346-365`, so no merge conflict)
**File**: `dashboard/src/components/VoiceOverPage.jsx`
**Changes**:
- `:40:14` `react-refresh/only-export-components`: the module exports a non-component alongside components. Move the constant into `dashboard/src/lib/` (a new tiny module or an existing one) and import it back, keeping every caller compiling.
- `:233:5` unused `eslint-disable` directive (suppresses nothing under the current rule set) — delete the directive line.

#### 4. dashboard/src/Legal.jsx — 1 finding
**File**: `dashboard/src/Legal.jsx`
**Changes**: `:84:40` unused arg `Icon` — underscore-prefix if callers pass it; drop from the signature otherwise.

#### 5. dashboard/src/components/PricingPage.jsx — 1 finding
**File**: `dashboard/src/components/PricingPage.jsx`
**Changes**: `:223:42` unused arg `Icon` — same policy as Legal.jsx.

#### 6. dashboard/src/components/RemotionPreview.jsx — 1 finding
**File**: `dashboard/src/components/RemotionPreview.jsx`
**Changes**: `:56:21` empty block statement — comment inside (with the reason) or delete the handler if dead.

### Success Criteria:

#### Automated Verification:
- [x] `cd dashboard && npm run lint` passes (strict, `--report-unused-disable-directives --max-warnings 0`) — repo-wide, the global gate this phase exists for
- [x] `cd dashboard && npm run build` passes
- [x] Review spot-check: `git diff` on the six files shows deletions confined to the inventory above — no live lines removed, no blanket disables added

#### Manual Verification:
- [ ] Subtitle editor still edits, saves, and burns word-timed captions — the deleted locals belonged to a superseded gap-anchoring algorithm; live caption timing must not shift
- [ ] Voice-style presets page loads, lists presets, and applies one
- [ ] Legal and Pricing pages render their sections normally
...
### Automated:
- Per-phase pytest targets: see each phase's Automated Verification block.
- Frontend phases: scoped strict lint over the phase's own files plus `npm run build`; the repo-wide `npm run lint` gate (strict, --max-warnings 0) lands with Phase 7.
- Pins stay green unmodified: `tests/test_no_double_route.py`, `tests/test_ai_provider.py`, `tests/test_llm_client.py`.

### Manual Testing Steps

1. `tests/test_no_double_route.py` (15 tests) must stay green UNMODIFIED except for added canaries — dispatch ownership never changes.
2. `tests/test_llm_endpoints.py`: all 26 existing tests stay green and unmodified; Slice 1 adds one `TestConfigFields` test pinning the new `/api/config` openai* fields.
3. `tests/test_ai_provider.py` (37, probe pins `:256-328`), `tests/test_llm_client.py` (56) stay green untouched.
4. Env namespace law: rerouted stages read `OPENAI_*`/job env or `resolve_openai`, never `LLM_*`.
5. Billing pin: cloud keeps `resolve_llm → None` (`app.py:193-194`) and the managed Gemini key (`app.py:143-150`); the thumbnail billing branch (`app.py:6058+`) is untouched.
6. Migration manual QA: fresh profile seeded with `llmConfig_v1` + `openai_*` + `geminiKey_v1` → import once, plaintext keys gone, reload does not resurrect them (writers removed in the same change).
...
8. Empty-model path: model field cleared → satellites fall back to server `LLM_MODEL`/`LLM_MODEL_<TASK>` (verify by hand; the rule is documentation-only in `lib/llm.js:9-11`).
9. Precedent hazards: guard every `env[...] = value` against None; never reword `"LLM provider"` error prefixes; audit the job gate each phase.
10. Cloud-mode smoke: `providerCfg` stays empty under billing; managed key still serves.
...
- Post-review restoration (plan assembly fix): Phase 1 app.py /api/config block, tests/test_llm_endpoints.py additive-test block, and the LlmProviderCard.jsx DELETE block; Phase 3/4 CLAUDE.md groups; Phase 4 garbage group removed. Restored verbatim from the design Architecture after a Files-line parse bug left placeholder groups. Success Criteria and phase boundaries untouched.


## References

- Design: `.rpiv/artifacts/designs/2026-09-10_07-09-56_unified-ai-provider-config.md`
- Solutions: `.rpiv/artifacts/solutions/2026-09-10_06-46-58_unified-ai-provider-config.md`
- Research: `.rpiv/artifacts/research/2026-09-09_20-00-08_unified-ai-provider-config.md`
- FRD (discover): `.rpiv/artifacts/discover/2026-09-09_19-17-18_unified-ai-provider-config.md`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| coverage | Testing Strategy 3 | <n/a> | concern | verification-coverage | Note "`tests/test_llm_client.py` (56) stay green untouched" — the `test_ai_provider.py` half lands in Phase 3/4/5 `#### Automated Verification:` bullets, but the `test_llm_client.py` pin appears in NO phase `### Success Criteria:` block and no code fence mirrors it; the plan-level `### Automated:` line is not a phase criterion, so a phase-by-phase implementer never runs it | Add "`pytest tests/test_llm_client.py -q` passes, file untouched" to Phase 1 `#### Automated Verification:` — the phase that lands the derived `X-LLM-*` emission the 56 tests pin | applied: AV bullet added to Phase 1 (`pytest tests/test_llm_client.py -q`) |
| coverage | Testing Strategy 5 | <n/a> | concern | verification-coverage | Note "cloud keeps `resolve_llm → None` (`app.py:193-194`)" — the thumbnail-billing-branch leg is mirrored (Phase 6 `not BILLING_ENABLED` gate) and the managed-key leg has manual bullets (Phases 5/6), but the `resolve_llm` billing leg has no Success Criteria bullet and no code mirror; criteria: NOT FOUND, code: NOT FOUND for this leg | Add a Phase 6 `#### Manual Verification:` bullet: "Cloud mode: `resolve_llm` still returns `None` (`app.py:193-194` unmodified) — satellite calls keep the managed path" | applied: MV bullet added to Phase 6 (`resolve_llm` → None billing leg) |
| coverage | Testing Strategy 8 | <n/a> | concern | verification-coverage | Note "model field cleared → satellites fall back to server `LLM_MODEL`/`LLM_MODEL_<TASK>` (verify by hand)" — the code mirror covers only the FE omission half (`if (model) headers['X-LLM-Model'] = model` in Phase 1 lib/llm.js); no Success Criteria bullet exercises the cleared-model → server-fallback outcome, and the note itself mandates a by-hand check that no manual bullet performs | Add a Phase 1 `#### Manual Verification:` bullet: "Card saved with base+key, model empty: a satellite request (e.g. titles) sends no `X-LLM-Model` and the server resolves `LLM_MODEL_<TASK>`" | applied: MV bullet added to Phase 1 (empty-model server fallback) |
| coverage | Testing Strategy 9 | <n/a> | concern | verification-coverage | Note "never reword `'LLM provider'` error prefixes" (`cloud/alerts.py` classifies on them) — no Success Criteria bullet and no code fence guard the prefixes; the sibling legs of the same note land (env writes via the `test_no_double_route` green pins, job gate via Phase 1's "the job starts" bullet), this leg does not | Add a Phase 6 `#### Automated Verification:` bullet: `git diff <Phase 1 base>..HEAD -- llm_client.py cloud/alerts.py` shows no modifications (the `'LLM provider'` prefixes stay byte-identical) | applied: AV bullet added to Phase 6 (`git diff --quiet -- llm_client.py cloud/alerts.py`) |

---

## Follow-up (2026-09-10T18:29:13+0700)

**Revision 1 — lint debt ownership** (triggered by the post-Phase-1 implementation run: repo-wide `npm run lint` fails on 32 pre-existing errors + 3 warnings at HEAD e5ffe7c, all in files no phase owned).

- Phase 7 added: pre-existing lint debt cleanup — 6 files, full per-file inventory with file:line refs, behavior-preserving fixes only (delete dead code; no blanket disables). `depends_on: []`; run right after Phase 1 in practice so later phases' scoped lint criteria converge on the global green.
- Phase 1's lint criterion amended to the scoped form over its four files and checked — evidence: the scoped strict run exited 0 post-implementation (this session).
- Phase 2's lint criterion amended to the scoped form over its four files until Phase 7 lands; the Testing Strategy frontend line updated to match.
- `VoiceOverPage.jsx`'s two findings stay in Phase 7 (sites `:40`/`:233` are outside Phase 2's edit regions `:84-86`/`:346-365` — no merge conflict).
- No other phases, criteria, or checkmarks touched.
